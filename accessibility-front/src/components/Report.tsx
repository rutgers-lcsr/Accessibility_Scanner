...
function Report({ report_id }: Props) {
  ...
  const {
    ...
    error,
    isLoading,
  } = useSWR(`/api/reports/${report_id}`, fetcherApi<ReportType>);
  ...
  const report_script_full_url =
    process.env.NODE_ENV === 'development'
      ? `http://localhost:5000/api/reports/${report_id}/script`
      : `${window.location.origin}/api/reports/${report_id}/script`;

  const report_photo_url =
    process.env.NODE_ENV === 'development'
      ? `http://localhost:5000/api/reports/${report_id}/photo`
      : `${window.location.origin}/api/reports/${report_id}/photo`;
    ...
    <Content className="">
      <header className="mb-8">
        ...
        <h2 className="mb-2 text-lg text-gray-500">
          Report Date:{' '}
          {reportData?.timestamp ? format(new Date(reportData.timestamp), 'MMMM dd, yyyy') : 'N/A'}
        </h2>
        ...
        >
          <h2 id="accessibility-report" className="mb-6 text-2xl font-semibold text-gray-800">
          ...
          {reportData.videos.length > 0 && (
            <div className="mt-4 text-center text-sm text-gray-600">
              ...
              <code>role=&quot;video&quot;</code> and <code>aria-label</code> attributes.{' '}
              <a
      ...
      <Content className="mb-8">
        ...
        <div>
          ...
          <p className="mb-2">
            To view the accessibility issues directly on the webpage, inject the following script
            into the browser console while on the page you want to audit:
          </p>
        </div>
        <Console
