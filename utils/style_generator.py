...
// Inside the preview frame the page URL carries the report URL percent-encoded.
if(currentUrl.includes(wanted) || currentUrl.includes(encodeURIComponent(wanted))) {{
  
    {base_js}

}} else {{
    accesslog(`The report URL does not match the current URL. Report URL: ${{wanted}}, Current URL: ${{currentUrl}}`, "warning");
