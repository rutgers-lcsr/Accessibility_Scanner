...
function OpenModal(title, message, extra = null, onClose = null) {
    ...
    const closeButton = modal.querySelector('.a11y-close-button');
    ...
    if (element) {
        ...
        aiButton.addEventListener('click', (e) => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                OpenModal('AI Prompt Copied to Clipboard', `The AI prompt has been copied to your clipboard. Please paste it into your preferred AI tool, such as Microsoft 365 Copilot, to get assistance with resolving the accessibility issue.`, extra);
                // wait 2 seconds
...
function placeTooltip(tooltip, element) {
    ...
    const viewportLeft = Math.max(VIEWPORT_MARGIN, Math.min(rect.left, window.innerWidth - width - VIEWPORT_MARGIN));
...
function createToolTip(injections, element, selector) {
    ...
    const a11yClasses = Array.from(clonedElement.classList).filter((cls) => cls.startsWith('a11y-'));
...
addAlert(`Click the "Ask Microsoft Copilot" button in the tooltip, this will copy the prompt to clipboard and open another tab with Microsoft 365 Copilot.`, 20000);
